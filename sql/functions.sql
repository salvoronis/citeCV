create or replace function get_schedule_marks(classid int, studentid int) returns table(dayoweek text, timee text, room int, subject text, mark int) as $$
	...
	where sch.class_id = $1 and sm.student_id = $2 order by sch.time
$$ language sql;
